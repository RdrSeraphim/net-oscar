...
is_deeply(
    [ sort keys(%Net::OSCAR::XML::xmlmap) ],
    [
        sort qw(
          just_byte
          just_word
...
is_deeply(
    ...
    [ { type => 'tlvchain', items => [ { type => 'data', num => 1, count => -1, name => 'x', items => [ { type => 'data', name => 'y' } ] } ] } ],
    "TLV chain, counted TLV"
);
is_deeply(
    [ protoparse( $oscar, "count_subtype_tlv" ) ],
    [ { type => 'tlvchain', subtyped => 1, items => [ { type => 'data', num => 1, subtype => 1, count => -1, name => 'x', items => [ { type => 'data', name => 'y' } ] } ] } ],
    "TLV chain, counted TLV"
);
