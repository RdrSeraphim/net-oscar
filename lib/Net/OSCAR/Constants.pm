...
use constant CONNTYPE_DIRECT_OUT =>
  dualvar( 0xff, "direct connect connection" );
...
"0x74 0x8F 0x24 0x20 0x62 0x87 0x11 0xD1 0x82 0x22 0x44 0x45 0x53 0x54 0x00 0x00"
    ...
    interoperate => {
        description => "ICQ/AIM interoperation",
        value       => pack(
            "C*",
            map { hex($_) } split(
                /[ \t\n]+/,
"0x09 0x46 0x13 0x4d 0x4c 0x7f 0x11 0xd1 0x82 0x22 0x44 0x45 0x53 0x54 0x00 0x00"
    ...
    extstatus => {
        description => "iChat extended status messages",
        value       => pack(
            "C*",
            map { hex($_) } split(
                /[ \t\n]+/,
...
"0x09, 0x46, 0x13, 0x41, 0x4c, 0x7f, 0x11, 0xd1, 0x82, 0x22, 0x44, 0x45, 0x53, 0x54, 0x00, 0x00"
    ...
    icq => {
        description => "EveryBuddy ICQ support",
        value       => pack(
            "C*",
            map { hex($_) } split(
                /[ \t\n,]+/,
...
"0x09, 0x46, 0x13, 0x4a, 0x4c, 0x7f, 0x11, 0xd1, 0x22, 0x82, 0x44, 0x45, 0x53, 0x54, 0x00, 0x00"
    ...
    sendlist => {
        description => "buddy list sending",
        value       => pack(
            "C*",
            map { hex($_) } split(
                /[ \t\n,]+/,
...
"0xaa, 0x4a, 0x32, 0xb5, 0xf8, 0x84, 0x48, 0xc6, 0xa3, 0xd7, 0x8c, 0x50, 0x97, 0x19, 0xfd, 0x5b"
    ...
    trilliancrypt => {
        description => "Trillian encryption",
        value       => pack(
            "C*",
            map { hex($_) } split(
                /[ \t\n,]+/,
"0xf2, 0xe7, 0xc7, 0xf4, 0xfe, 0xad, 0x4d, 0xfb, 0xb2, 0x35, 0x36, 0x79, 0x8b, 0xdf, 0x00, 0x00"
    ...
    secureim => {
        description => "SecureIM encryption",
        value       => pack(
            "C*",
            map { hex($_) } split(
                /[ \t\n,]+/,
...
use constant OSCAR_CAPS_INVERSE =>
  { map { OSCAR_CAPS()->{$_}->{value} => $_ } keys %{ OSCAR_CAPS() } };
use constant OSCAR_CAPS_SHORT_INVERSE =>
  { map { substr( OSCAR_CAPS()->{$_}->{value}, 2, 2 ) => $_ }
      keys %{ OSCAR_CAPS() } };
...
use constant BUDTYPES => (
    "buddy",                 "group",
    "permit entry",          "deny entry",
    "visibility/misc. data", "presence",
    undef,                   undef,
    undef,                   undef,
    undef,                   undef,
    undef,                   undef,
...
use constant ICQ_META_INFO_INVERSE =>
  { map { ICQ_META_INFO()->{$_} => $_ } keys %{ ICQ_META_INFO() } };
