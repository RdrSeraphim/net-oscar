...
sub process_snac($$) {
    ...
    if ( !exists( $protohandlers{$protobit} ) ) {
        $protohandlers{$protobit} = eval { require "Net/OSCAR/ServerCallbacks/$family/$protobit.pm"; };
        if ($@) {
            my $olderr = $@;
            $protohandlers{$protobit} = eval { require "Net/OSCAR/ServerCallbacks/0/$protobit.pm"; };
        }
    }
