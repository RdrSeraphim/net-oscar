...
our %COOKIES;
$SCREENNAMES->{somedude} = {
    sn    => "Some Dude",
    pw    => "somepass",
    email => 'some@dude.com',
    blist => [qw(SomeDude OtherDude)]
};
$SCREENNAMES->{otherdude} = {
    sn    => "Other Dude",
    pw    => "otherpass",
    email => 'other@dude.com',
    blist => [qw(SomeDude OtherDude)]
};
...
sub process_snac($$) {
    ...
    if ( !exists( $protohandlers{$protobit} ) ) {
        $protohandlers{$protobit} =
          eval { require "Net/OSCAR/ServerCallbacks/$family/$protobit.pm"; };
        if ($@) {
            my $olderr = $@;
            $protohandlers{$protobit} =
              eval { require "Net/OSCAR/ServerCallbacks/0/$protobit.pm"; };
        }
    }
