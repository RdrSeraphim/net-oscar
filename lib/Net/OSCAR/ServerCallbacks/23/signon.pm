...
sub {
    ...
    my @valid_hashes = map { [ $_, encode_password( $session, exists( $data{pass_is_hashed} ) ? md5( $SCREENNAMES->{$screenname}->{pw} ) : $SCREENNAMES->{$screenname}->{pw}, $_ ) ]; } keys %{ $SESSIONS->{$screenname}->{keys} };
    ...
    if ($valid) {
        ...
        $connection->proto_send(
            protobit  => "authorization_response",
            protodata => {
