...
sub blentry_clear($%) {
    ...
    if (
        chain_exists(
            $session->{blinternal}, $data{entry_type},
            $data{group_id},        $data{buddy_id}
        )
      )
    {
        $session->{blinternal}->{ $data{entry_type} }->{ $data{group_id} }
          ->{ $data{buddy_id} }->{__BLI_DELETED} = 1;
    }
}
...
sub blentry_set($%) {
    ...
    $session->{blinternal}->{ $data{entry_type} }->{ $data{group_id} }
      ->{ $data{buddy_id} }->{name} = $data{entry_name}
      if $data{entry_name};
    while ( my ( $key, $value ) = each %$typedata ) {
        $session->{blinternal}->{ $data{entry_type} }->{ $data{group_id} }
          ->{ $data{buddy_id} }->{data}->{$key} = $value;
    }
    $session->log_printf_cond(
        OSCAR_DBG_DEBUG,
        sub {
            "Got BLI entry %s 0x%04X/0x%04X/0x%04X with %d bytes of data:%s",
              $data{entry_name}, $data{entry_type}, $data{group_id},
              $data{buddy_id}, length($typedata), hexdump( $data{entry_data} );
        }
    );
}
...
sub blparse($$) {
    ...
    $session->{visibility} =
      VISMODE_PERMITALL;    # If we don't have p/d data, this is default.
    ...
    while ( length($data) > 4 ) {
        ...
        my ( $gid, $bid, $type, $sublen ) =
          unpack( "n4", substr( $data, 0, 8, "" ) );
...
sub BLI_to_NO($) {
    ...
    foreach my $type ( 4, 5, 0x14 ) {
        delete $bli->{$type}->{0}->{$_}
          foreach grep { $bli->{$type}->{0}->{$_}->{__BLI_DELETED} }
          keys %{ $bli->{$type}->{0} };
    }
    ...
    if (
        ...
        and exists $bli->{4}->{0}
        and ( my ($visbid) =
            grep { exists( $bli->{4}->{0}->{$_}->{data}->{0xCB} ) }
            keys %{ $bli->{4}->{0} } )
      )
    {
        ...
        if ( $bli->{4}->{0}->{$visbid}->{__BLI_DIRTY} ) {
            ( $session->{visibility} ) = unpack( "C", $typedata->{0xCA} )
              if $typedata->{0xCA};

            my $groupperms = $typedata->{0xCB};
            ( $session->{groupperms} ) = unpack( "N", $groupperms )
              if $groupperms;
            $session->{profile} = $typedata->{0x0100}
              if exists( $typedata->{0x0100} );
            ( $session->{icon_checksum} ) = unpack( "n", $typedata->{0x0101} )
              if exists( $typedata->{0x0101} );
            ( $session->{icon_timestamp} ) = unpack( "N", $typedata->{0x0102} )
              if exists( $typedata->{0x0102} );
            ( $session->{icon_length} ) = unpack( "N", $typedata->{0x0103} )
              if exists( $typedata->{0x0103} );

            $session->{appdata} = $typedata;

            $session->set_info( $session->{profile} )
              if exists( $session->{profile} );
    ...
    if (
            exists $bli->{0x14}
        and exists $bli->{0x14}->{0}
        and ( my ($iconbid) =
            grep { exists( $bli->{0x14}->{0}->{$_}->{data}->{0xD5} ) }
            keys %{ $bli->{0x14}->{0} } )
      )
    {
    ...
    foreach my $gid ( keys %{ $bli->{1} } ) {
        ...
        if ( $item->{__BLI_DELETED} ) {
            ...
            push @ret,
              {
                type   => MODBL_WHAT_GROUP,
                action => MODBL_ACTION_DEL,
                group  => $item->{name}
              };
        }
        elsif ( $item->{__BLI_DIRTY} ) {
    ...
    foreach my $gid ( keys %{ $bli->{0} } ) {
        foreach my $bid ( keys %{ $bli->{0}->{$gid} } ) {
            ...
            my $group = "";
            $group = $bli->{1}->{$gid}->{0}->{name}
              if chain_exists( $bli, 1, $gid, 0 );

            if ( $item->{__BLI_DELETED} ) {
                delete $bli->{0}->{$gid}->{$bid};
                next if $gid == 0 or !$group;

                delete $session->{buddies}->{$group}->{members}
                  ->{ $item->{name} }
                  if $group;
                push @ret,
                  {
                    type   => MODBL_WHAT_BUDDY,
                    action => MODBL_ACTION_DEL,
                    group  => $group,
                    buddy  => $item->{name}
                  };
                ...
                my $comment = undef;
                $comment = $item->{data}->{0x13C}
                  if exists( $item->{data}->{0x13C} );

                my $alias = undef;
                $alias = $item->{data}->{0x131}
                  if exists( $item->{data}->{0x131} );
                ...
                $entry->{data}          = $item->{data};
                $entry->{screenname} =
                  Net::OSCAR::Screenname->new( $item->{name} );
...
sub NO_to_BLI($) {
    ...
    foreach my $type ( keys %$oldbli ) {
        ...
        foreach my $gid ( keys %{ $oldbli->{$type} } ) {
            foreach my $bid ( keys %{ $oldbli->{$type}->{$gid} } ) {
                ...
                $bli->{$type}->{$gid}->{$bid}->{name} =
                  $oldbli->{$type}->{$gid}->{$bid}->{name};
                foreach my $data (
                    keys %{ $oldbli->{$type}->{$gid}->{$bid}->{data} } )
                {
                    $bli->{$type}->{$gid}->{$bid}->{data}->{$data} =
                      $oldbli->{$type}->{$gid}->{$bid}->{data}->{$data};
                }
            }
        }
    }
    ...
    foreach my $permit ( keys %{ $session->{permit} } ) {
        ...
        $bli->{2}->{0}->{ $session->{permit}->{$permit}->{buddyid} }->{name} =
          $permit;
    }

    foreach my $deny ( keys %{ $session->{deny} } ) {
        init_entry( $bli, 3, 0, $session->{deny}->{$deny}->{buddyid} );
        $bli->{3}->{0}->{ $session->{deny}->{$deny}->{buddyid} }->{name} =
          $deny;
    }

    init_entry( $bli, 4, 0, $visbid );
    $bli->{4}->{0}->{$visbid}->{data}->{0xCA} =
      pack( "C", $session->{visibility} || VISMODE_PERMITALL );
    $bli->{4}->{0}->{$visbid}->{data}->{0xCB} =
      pack( "N", $session->{groupperms} || 0xFFFFFFFF );

    #Net::OSCAR protocol extensions
    $bli->{4}->{0}->{$visbid}->{data}->{0x0100} = $session->{profile}
      if $session->{profile};
    $bli->{4}->{0}->{$visbid}->{data}->{0x0101} =
      pack( "n", $session->{icon_checksum} )
      if $session->{icon_checksum};
    $bli->{4}->{0}->{$visbid}->{data}->{0x0102} =
      pack( "N", $session->{icon_timestamp} )
      if $session->{icon_timestamp};
    $bli->{4}->{0}->{$visbid}->{data}->{0x0103} =
      pack( "N", $session->{icon_length} )
      if $session->{icon_length};

    foreach my $appdata ( keys %{ $session->{appdata} } ) {
        $bli->{4}->{0}->{$visbid}->{data}->{$appdata} =
          $session->{appdata}->{$appdata};
    }

    if ( exists( $session->{icon_md5sum} )
        || chain_exists( $oldbli, 0x14, 0, $iconbid ) )
    {
        init_entry( $bli, 0x14, 0, $iconbid );

        if ( chain_exists( $oldbli, 0x14, 0, $iconbid ) ) {
            $bli->{0x14}->{0}->{$iconbid}->{name} =
              $oldbli->{0x14}->{0}->{$iconbid}->{name};

            $bli->{0x14}->{0}->{$iconbid}->{data}->{$_} =
              $oldbli->{0x14}->{0}->{$iconbid}->{data}->{$_}
              foreach grep { $_ != 0xD5 }
              keys %{ $oldbli->{0x14}->{0}->{$iconbid}->{data} };
        }
        else {
    ...
    if ( $session->{buddies}->{__BLI_DIRTY} ) {
        $bli->{1}->{0}->{0}->{data}->{0xC8} = pack( "n*",
            map { $_->{groupid} }
            grep { ref($_) } values %{ $session->{buddies} } );
        $session->{buddies}->{__BLI_DIRTY} = 0;
    }
    else {
...
# It seems that WinAIM can now have groups without 0xC8 data, and gets pissed if we create such data where it doesn't exist.
        if ( !exists( $oldbli->{1}->{$gid} )
            or chain_exists( $oldbli, 1, $gid, 0, "data", 0xC8 ) )
        {
        ...
        if ( chain_exists( $oldbli, 1, $gid, 0 ) ) {
            $bli->{1}->{$gid}->{0}->{data}->{$_} =
              $oldbli->{1}->{$gid}->{0}->{data}->{$_}
              foreach grep { $_ != 0xC8 }
              keys %{ $oldbli->{1}->{$gid}->{0}->{data} };
        }
        ...
        while ( my ( $buddy, $bud ) = each( %{ $grp->{members} } ) ) {
            ...
            $blibud->{name} =
              "$buddy";    # Make sure to get strinfied version of Screenname

            $blibud->{data} = tlv();
            while ( my ( $key, $value ) = each( %{ $bud->{data} } ) ) {
                $blibud->{data}->{$key} = $value;
            }
            $blibud->{data}->{0x13C} = $bud->{comment}
              if defined $bud->{comment};
...
sub BLI_to_OSCAR($$) {
    ...
    $session->crapout(
        $session->{services}->{ 0 + CONNTYPE_BOS },
"You must wait for a buddylist_ok or buddylist_error callback before calling commit_buddylist again."
    ) if $session->{budmods};
    $session->{budmods} = [];
        ...
        foreach my $gid ( keys %{ $oldbli->{$type} } ) {
            foreach my $bid ( keys %{ $oldbli->{$type}->{$gid} } ) {
                ...
                $session->log_printf_cond(
                    OSCAR_DBG_DEBUG,
                    sub {
"Old BLI entry %s 0x%04X/0x%04X/0x%04X with %d bytes of data:%s",
                          $oldentry->{name}, $type, $gid, $bid,
                          length($olddata), hexdump($olddata);
                    }
                );
                my $delete = 0;
                if (    exists( $newbli->{$type} )
                    and exists( $newbli->{$type}->{$gid} )
                    and exists( $newbli->{$type}->{$gid}->{$bid} ) )
                {
                    ...
                    $session->log_printf_cond(
                        OSCAR_DBG_DEBUG,
                        sub {
"New BLI entry %s 0x%04X/0x%04X/0x%04X with %d bytes of data:%s",
                              $newentry->{name}, $type, $gid, $bid,
                              length($newdata), hexdump($newdata);
                        }
                    );

                    next
                      if $newentry->{name} eq $oldentry->{name}
                    ...
                    if ( $newentry->{name} ne $oldentry->{name} ) {
                        ...
                        push @{ $budmods{modify} },
                          {
                            reqdata => {
                                desc => "modifying $budtype $newentry->{name}",
                                type => $type,
                                gid  => $gid,
                                bid  => $bid
                            },
                ...
                if ($delete) {
                    ...
                    push @{ $budmods{delete} },
                      {
                        reqdata => {
                            desc => "deleting $budtype $oldentry->{name}",
                            type => $type,
                            gid  => $gid,
                            bid  => $bid
                        },
    ...
    foreach my $type ( keys %$newbli ) {
        ...
        foreach my $gid ( keys %{ $newbli->{$type} } ) {
            foreach my $bid ( keys %{ $newbli->{$type}->{$gid} } ) {
                ...
                next
                  if exists( $oldbli->{$type} )
                  and exists( $oldbli->{$type}->{$gid} )
                  and exists( $oldbli->{$type}->{$gid}->{$bid} )
                  and $oldbli->{$type}->{$gid}->{$bid}->{name} eq
                  $newbli->{$type}->{$gid}->{$bid}->{name};
                ...
                $session->log_printf_cond(
                    ...
                    sub {
"New BLI entry %s 0x%04X/0x%04X/0x%04X with %d bytes of data:%s",
                          $entry->{name}, $type, $gid, $bid, length($data),
                          hexdump($data);
                    }
                );
                ...
                push @{ $budmods{add} },
                  {
                    reqdata => {
                        desc => "adding $budtype $entry->{name}",
                        type => $type,
                        gid  => $gid,
                        bid  => $bid
                    },
...
# FLAP size limit is 8K, but that includes headers - good to have a safety margin
    foreach my $type (qw(add modify delete)) {
        ...
        foreach my $change (@$changelist) {
            $packet .= protoparse( $session, "buddylist_modification" )
              ->pack( %{ $change->{protodata} } );
...
#$session->log_print(OSCAR_DBG_INFO, "Adding to blmod queue (no more changes): type $type, payload size ", scalar(@reqdata));
    ...
    push @{ $session->{budmods} },
      { protobit => "end_buddylist_modifications" };    # End BL mods
      #$session->log_print(OSCAR_DBG_INFO, "Adding terminator to blmod queue.");

    $session->{blold}      = $oldbli;
    $session->{blinternal} = $newbli;

    if ( @{ $session->{budmods} } <= 1 )
    {    # We only have the start/end modification packets, no actual changes
...
#$session->log_print(OSCAR_DBG_INFO, "Non-empty blmod queue - sending initiator and first change packet.");
        $session->svcdo( CONNTYPE_BOS,
            protobit => "start_buddylist_modifications" );
        $session->svcdo( CONNTYPE_BOS, %{ shift @{ $session->{budmods} } } )
          ;    # Send the first modification
    }
}
