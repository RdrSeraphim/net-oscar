...
sub NO_to_BLI($) {
    ...
    if ( exists( $session->{icon_md5sum} ) || chain_exists( $oldbli, 0x14, 0, $iconbid ) ) {
        ...
        if ( chain_exists( $oldbli, 0x14, 0, $iconbid ) ) {
            ...
            $bli->{0x14}->{0}->{$iconbid}->{data}->{$_} = $oldbli->{0x14}->{0}->{$iconbid}->{data}->{$_} foreach grep { $_ != 0xD5 } keys %{ $oldbli->{0x14}->{0}->{$iconbid}->{data} };
        }
        else {
    ...
    while ( my ( $grpname, $grp ) = each( %{ $session->{buddies} } ) ) {
        ...
        if ( !exists( $oldbli->{1}->{$gid} ) or chain_exists( $oldbli, 1, $gid, 0, "data", 0xC8 ) ) {
            $bligrp->{data}->{0xC8} = pack(
                "n*",
                map    { $_->{buddyid} }
                  grep { not $_->{__BLI_DELETED} } values %{ $grp->{members} }
            );
        }

        if ( chain_exists( $oldbli, 1, $gid, 0 ) ) {
            $bli->{1}->{$gid}->{0}->{data}->{$_} = $oldbli->{1}->{$gid}->{0}->{data}->{$_} foreach grep { $_ != 0xC8 } keys %{ $oldbli->{1}->{$gid}->{0}->{data} };
        }
